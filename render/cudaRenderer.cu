...
#define SCAN_BLOCK_DIM 1024 // 32 * 32
...
__global__ void kernelRenderPixels() {
    ...
    bool isValidPixel = (pixelX < imageWidth && pixelY < imageHeight);
    float4 img;
    if (isValidPixel) {
        img = *((float4*)(&cuConstRendererParams.imageData[4 * (pixelY * imageWidth + pixelX)]));
    }

    // block image bounding box
    float boxL = float(blockIdx.x * blockDim.x) / imageWidth;
    float boxR = min(float(blockIdx.x + 1) * blockDim.x / imageWidth, 1.0);
    float boxB = float(blockIdx.y * blockDim.y) / imageHeight;
    float boxT = min(float(blockIdx.y + 1) * blockDim.y /imageHeight, 1.0);
    // use block-level shared memory to independently check whether a circle intersects the block bounding box
    ...
    __shared__ uint numOverlaps;
    __shared__ uint overlaps[SCAN_BLOCK_DIM];
    int threadIndex = threadIdx.y * blockDim.y + threadIdx.x;
    for (int index = 0; index < cuConstRendererParams.numberOfCircles; index += SCAN_BLOCK_DIM) {
        int circleIdx = index + threadIndex;
        ...
        if (circleIdx < cuConstRendererParams.numberOfCircles) {
            ...
            prefixSumInput[threadIndex] = circleInBox(p.x, p.y, rad, boxL, boxR, boxT, boxB);
        } else {
            prefixSumInput[threadIndex] = 0;
        }
        __syncthreads();

        sharedMemExclusiveScan(threadIndex, prefixSumInput, prefixSumOutput, prefixSumScratch, SCAN_BLOCK_DIM);
        __syncthreads();

        if (threadIndex == SCAN_BLOCK_DIM - 1) {
            // special case: handle the last thread in block
            // this is because prefixSumInput[SCAN_BLOCK_DIM - 1] is excluded from the output
            numOverlaps = prefixSumOutput[threadIndex];
            if (prefixSumInput[threadIndex] == 1) {
                overlaps[numOverlaps] = circleIdx;
                numOverlaps++;
            }
        } else if (prefixSumOutput[threadIndex] + 1 == prefixSumOutput[threadIndex+1]) {
            // circle intersects the block
            overlaps[prefixSumOutput[threadIndex]] = circleIdx;
        }
        __syncthreads();

        // iterator through overlapped circles and render for pixels
        for (int k = 0; k < numOverlaps; k++) {
            circleIdx = overlaps[k];
            index3 = 3 * circleIdx;
            p = *(float3*)(&cuConstRendererParams.position[index3]);
            // shade pixel
            shadePixel(pixelCenterNorm, p, &img, circleIdx);
        }
    }
    // render for valid pixel only
    if (isValidPixel) {
        *((float4*)(&cuConstRendererParams.imageData[4 * (pixelY * imageWidth + pixelX)])) = img;
    }
}
